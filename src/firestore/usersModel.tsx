...
class UserModel extends FirestoreModel {
  ...
  public async userExists(userId: string) {
    const docRef = doc(this.store, `${this.main}/${this.collection}/${userId}`, 'userInfo');
    const docSnap = await getDoc(docRef);
    return docSnap.exists();
  }

  public async addUser(userId: string, data: User) {
    setDoc(doc(this.store, `${this.main}/${this.collection}/${userId}`, 'userInfo'), data);
  }

  public async updateUser(userId: string, data: UpdateUser) {
    const userRef = doc(this.store, `${this.main}/${this.collection}/${userId}`, 'userInfo');
  ...
  public addUserListener(userId: string, updateInboxRoomData: (data: DocumentData | undefined) => null) {
    const unsubscribe = onSnapshot(doc(this.store, `${this.main}/${this.collection}/${userId}`, 'userInfo'), (d) => {
